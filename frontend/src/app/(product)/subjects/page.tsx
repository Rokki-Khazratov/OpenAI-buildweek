...
export default function SubjectsPage() {
  ...
  return (
    <PageFrame eyebrow={`${subjects.length} subjects · ${subjects.reduce((total, subject) => total + subject.examCount, 0)} exams`} title="Subjects" action={<Link href="/subjects/new" className="inline-flex min-h-10 items-center gap-2 rounded-[9px] bg-signal px-4 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(46,46,255,0.18)]"><Plus size={16} /> New subject</Link>}>
      ...
      {visible.length ? (
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{visible.map((subject) => <SubjectCard key={subject.id} subject={subject} />)}</div>
      ) : (
        <div className="grid min-h-[320px] place-items-center rounded-[14px] border border-dashed border-line bg-surface-raised p-8 text-center"><div><p className="font-semibold">No subjects match “{query}”</p><p className="mt-2 text-sm text-muted">Try a different title, institution or course code.</p></div></div>
      )}
    </PageFrame>
  );
}
