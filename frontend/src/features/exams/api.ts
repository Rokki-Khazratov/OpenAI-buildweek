...
export type AttemptSummaryDto = { attempt_id: string; exam_id: string; score: number; max_score: number; duration_seconds: number; submitted_at: string; feedback: string };

const payload = (input: ExamInput) => ({ title: input.title, description: input.description || null, exam_type: input.examType || null, language: input.language, target_date: input.targetDate || null, pasted_context: input.pastedContext, sources: [], blueprint: input.blueprint, rules: input.rules, scenario: input.scenario });
