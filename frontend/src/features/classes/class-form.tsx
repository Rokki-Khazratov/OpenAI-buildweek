  ...
  return (
    <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,680px)_280px]">
      <section className="rounded-[14px] border border-line bg-white p-5 sm:p-7">
        ...
        <div className="mt-8 flex flex-col-reverse gap-3 border-t border-line pt-6 sm:flex-row sm:justify-between"><Link href={studyClass ? `/classes/${studyClass.id}` : "/classes"} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[9px] px-3 text-sm font-semibold text-muted hover:bg-surface"><ArrowLeft size={16} /> Cancel</Link><Button type="submit" disabled={pending || !effectiveSubjectId || (examScope === "selected" && selectedExamIds.length === 0)}>{pending ? "Saving…" : studyClass ? "Save changes" : "Create class"}{!pending && <ArrowRight size={16} />}</Button></div>
      </section>
      <aside className="h-fit rounded-[14px] border border-line bg-surface-raised p-5"><p className="text-sm font-semibold">How classes work</p><p className="mt-2 text-xs leading-5 text-muted">A class shares the selected subject scope with existing ExamTwin accounts. Add participants after creation.</p><div className="mt-5 rounded-[10px] border border-line bg-white p-3.5 text-xs leading-5 text-muted"><strong className="font-semibold text-ink">Private cohort</strong><br />Only the owner sees aggregate readiness and weak-skill analytics. Raw answers are never exposed.</div></aside>
    </form>
  );
}
