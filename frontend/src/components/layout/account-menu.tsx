  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((current) => !current)} aria-expanded={open} className="flex min-h-10 items-center gap-2 rounded-[10px] px-1.5 transition hover:bg-surface sm:px-2.5">
        <span className="grid size-8 place-items-center rounded-full bg-contrast text-xs font-semibold text-contrast-ink">RK</span>
        <span className="hidden text-left md:block"><span className="block text-xs font-semibold leading-4">Rokki</span><span className="block text-[10px] leading-3 text-muted">TU Wien</span></span>
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-[46px] z-50 w-[220px] rounded-[12px] border border-line bg-canvas p-1.5 shadow-float">
            <div className="border-b border-line px-3 py-2.5"><p className="text-sm font-semibold">Rokki Khazratov</p><p className="mt-0.5 truncate text-xs text-muted">rokki@example.com</p></div>
