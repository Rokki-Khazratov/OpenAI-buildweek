...
if (existing.items.some((item) => item.title === catalog[0][0])) {
  console.log(JSON.stringify({ status: "already_seeded", credentials }));
  process.exit(0);
}

let totals = { subjects: 0, exams: 0, classes: 0, mocks: 0, attempts: 0 };
for (const [title, code, examTitles] of catalog) {
...
console.log(JSON.stringify({ status: "seeded", credentials, ...totals }));
